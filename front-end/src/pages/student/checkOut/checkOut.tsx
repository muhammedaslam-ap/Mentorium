...
const stripePromise = loadStripe(import.meta.env.VITE_STRIPE_PUBLISH_KEY || "pk_test_placeholder");
...
const RenderStripe = ({ courseId, amount, isProcessing, setIsProcessing, onPaymentSuccess }: RenderStripeProps) => {
  ...
  const handleStripePayment = async () => {
    if (!stripe || !elements) {
      console.error("Stripe initialization error: stripe or elements not available");
    ...
    try {
      console.log("Step 1: Creating PaymentIntent", { courseId, amount: amount * 100, currency: "inr" });
      ...
      if (result.error) {
        ...
        const isIndianRegulationError = result.error.message?.includes("export transactions require a description");
        if (!isIndianRegulationError) {
          setError(result.error.message || "Payment failed. Please try again.");
          toast.error(result.error.message || "Payment failed");
        } else {
          throw new Error("Payment failed due to Indian regulation: export transactions require a description");
        }
        ...
        try {
          console.log("Step 3: Updating payment status", { paymentIntentId: result.paymentIntent.id });
          const updateResponse = await authAxiosInstance.post("/payment/stripe", {
            paymentIntentId: result.paymentIntent.id,
            status: "succeeded",
          });
          console.log("Step 3 Response: Payment status updated", updateResponse.data);

          console.log("Step 4: Enrolling user", { courseId, transactionId: result.paymentIntent.id, amount });
          ...
          console.error("Step 3/4 Error: Enrollment failed after payment", error);
      ...
      const isIndianRegulationError = error.message?.includes("export transactions require a description");
      if (!isIndianRegulationError) {
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-md border border-gray-200 p-4 dark:border-gray-700">
        <div className="mb-4">
          <label htmlFor="card-element" className="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">
            Card Details
          </label>
...
const CheckoutPage = () => {
  ...
  }, [enrollmentStatus, courseId, navigate]);
  ...
  const handlePaymentSuccess = (transactionId: string) => {
    ...
    console.log(`Generating PDF receipt for transaction: ${transactionId}, course: ${course?.title}, amount: ${course?.price}, at 08:15 PM IST on 19 May 2025`);
      ...
      <div className="min-h-screen bg-white dark:bg-gray-900">
        ...
        <div className="container mx-auto max-w-6xl px-4 py-12 md:py-16">
          <div className="flex flex-col items-center justify-center space-y-4">
            ...
            <p className="text-lg font-medium text-gray-600 dark:text-gray-300">Loading course details...</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="min-h-screen bg-white dark:bg-gray-900">
        ...
        <div className="container mx-auto max-w-6xl px-4 py-12 md:py-16">
          <div className="flex flex-col items-center justify-center text-center">
            ...
            <h2 className="mb-3 text-2xl font-bold text-gray-800 dark:text-gray-100">Course Not Found</h2>
        ...
        {enrollmentStatus !== "enrolled" && (
          <div className="mb-8">
            <div className="mb-2 flex justify-between px-2">
              <span className={`text-sm font-medium ${paymentStep >= 1 ? 'text-blue-600 dark:text-blue-400' : 'text-gray-500 dark:text-gray-400'}`}>
                Course Details
              </span>
              <span className={`text-sm font-medium ${paymentStep >= 2 ? 'text-blue-600 dark:text-blue-400' : 'text-gray-500 dark:text-gray-400'}`}>
                Payment Method
              </span>
              <span className={`text-sm font-medium ${paymentStep >= 3 ? 'text-blue-600 dark:text-blue-400' : 'text-gray-500 dark:text-gray-400'}`}>
                Complete Payment
              </span>
            </div>
            <Progress 
            ...
            <Card className="overflow-hidden bg-white shadow-md dark:bg-gray-800 dark:border-gray-700">
              <CardHeader className="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-gray-800 dark:to-gray-750 border-b border-gray-100 dark:border-gray-700 pb-4">
                <CardTitle className="text-xl font-bold text-gray-800 dark:text-white">
                  {enrollmentStatus === "enrolled" ? "You're Enrolled!" : "Secure Checkout"}
                </CardTitle>
              ...
              <CardContent className="p-6">
                {enrollmentStatus === "enrolled" ? (
                  <div className="flex flex-col items-center text-center">
                    ...
                    <h2 className="mb-3 text-2xl font-bold text-gray-800 dark:text-white">You're Already Enrolled!</h2>
                            ...
                            <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
                              <div className="mb-6 flex items-center justify-center">
                                <img
                                  ...
                                  className="h-10"
                                  onError={(e) => {
                                    (e.target as HTMLImageElement).src = "/placeholder.svg?height=40&width=120&text=Stripe";
                                  }}
                                />
                              </div>
                              
                              <p className="mb-6 text-center text-gray-600 dark:text-gray-300">
                                Secure payment processing with Stripe. Your card details are encrypted and secure.
                              </p>
                ...
                <CardFooter className="flex justify-between border-t border-gray-200 bg-gray-50 p-6 dark:border-gray-700 dark:bg-gray-800">
                  ...
                  >
                    {isProcessing ? (
                      ...
                      <>
                        {paymentStep === 1 ? "Continue to Payment" : "Proceed to Checkout"}
                      </>
                    )}
                  </Button>
                </CardFooter>
              ...
              <CardContent className="p-6">
                <div className="space-y-4">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-gray-800 dark:text-white">₹{course.price.toFixed(2)}</span>
                  </div>
                  ...
                  <div className="flex justify-between font-medium">
                    ...
                    <span className="text-lg text-blue-600 dark:text-blue-400">₹{course.price.toFixed(2)}</span>
                  </div>
                </div>
              </CardContent>
              
              <div className="border-t border-gray-100 p-6 dark:border-gray-700">
                <h3 className="mb-4 text-sm font-medium text-gray-800 dark:text-white">What you'll get:</h3>
                <ul className="space-y-3 text-sm">
                  <li className="flex items-start">
                    <CheckCircle className="mr-3 h-5 w-5 text-green-500 flex-shrink-0" />
                    <span className="text-gray-600 dark:text-gray-300">Full lifetime access to this course</span>
                  </li>
                  <li className="flex items-start">
                    <CheckCircle className="mr-3 h-5 w-5 text-green-500 flex-shrink-0" />
                    <span className="text-gray-600 dark:text-gray-300">
                      {loadingLessons ? "Loading..." : `${lessons.length} on-demand video lessons`}
                    </span>
                  </li>
                  <li className="flex items-start">
                    <CheckCircle className="mr-3 h-5 w-5 text-green-500 flex-shrink-0" />
        ...
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription className="text-center text-gray-600 dark:text-gray-300">
              Congratulations! You've successfully enrolled in <span className="font-semibold">{course.title}</span>. A receipt has been generated for your records. Start learning now!
            </DialogDescription>
          </DialogHeader>
